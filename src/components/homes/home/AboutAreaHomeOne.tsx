...
import type { HomeAboutArea, HomeAboutImpact, HomeAboutClients } from '@/services/home/home.types';
...
interface Props {
  ...
  hideImpact?: boolean;
  data?: HomeAboutArea;
  impact?: HomeAboutImpact;
  clients?: HomeAboutClients;
}

const AboutAreaHomeOne = ({ hideClients, hideImpact, data, impact, clients }: Props) => {
  // About block
  const aboutImage = data?.image || about_thumb;
  const heading = data?.heading || 'How Aquabrim Water Level Controller Works';
  const paragraphs = data?.paragraphs?.length ? data.paragraphs : DEFAULT_PARAGRAPHS;
  const features = data?.features?.length ? data.features : DEFAULT_FEATURES;
  const ctaBtnText = data?.ctaBtnText || 'View Products';
  const ctaBtnLink = data?.ctaBtnLink || '/products';

  // Impact block
  const impactEyebrow = impact?.eyebrow || 'Trusted by thousands';
  const impactHeading = impact?.heading || 'Trusted by 50,000+ Customers Across India';
  const impactSubheading = impact?.subheading || 'Installed across residential, commercial and institutional';
  const stats = impact?.stats?.length ? impact.stats : DEFAULT_STATS;

  // Clients block
  const clientsHeading = clients?.heading || 'They trust Aquabrim';
  const clientLogos = clients?.logos?.length ? clients.logos : DEFAULT_CLIENTS;
      ...
      <div className="container-app">
        <div className="grid grid-cols-1 items-center gap-8 lg:grid-cols-2">
          ...
          <div className="hidden lg:block">
            <Image src={aboutImage} width={640} height={480} style={{ width: "100%", height: "auto", borderRadius: "8px" }} alt="Aquabrim Installation" />
          </div>
          ...
          <div>
            ...
            <div className="mb-4 text-center lg:text-left">
              {paragraphs.map((p, i) => (
                <p key={i} className="mb-2 text-[0.95rem] leading-[1.7] text-gray-500">{p}</p>
              ))}
            </div>

            {/* Mobile-only image */}
            <div className="mx-auto mb-12 mt-6 block max-w-[480px] lg:hidden">
              <Image src={aboutImage} width={480} height={360} style={{ height: "auto", borderRadius: "8px", width: "100%" }} alt="Aquabrim Installation" />
            </div>
            ...
            <div className="text-center lg:text-left">
              <Link href={ctaBtnLink} className="inline-flex items-center rounded-md bg-primary px-4 py-3 font-bold text-white no-underline shadow-sm transition-colors hover:bg-primary-600">
                {ctaBtnText} <i className="bi bi-arrow-right ml-2"></i>
              </Link>
            </div>
          </div>
        </div>
            ...
            {!hideImpact && (
              <>
                <div className="mb-10 text-center">
                  <span className="mb-2 block text-[0.9rem] font-bold uppercase tracking-[1.5px] text-primary">{impactEyebrow}</span>
                  <h3 className="mb-2 text-[clamp(1.375rem,1.375rem+1.5vw,2.5rem)] font-bold text-[#0d1b2a]">{impactHeading}</h3>
                  <p className="text-lg text-gray-500">{impactSubheading}</p>
                </div>
            ...
            {!hideClients && (
              <>
                ...
                <div className="mb-4 text-center">
                  <h6 className="text-[clamp(22px,5vw,28px)] font-bold uppercase tracking-[1.5px] text-gray-500">{clientsHeading}</h6>
                </div>

                <div className="marquee-wrapper relative w-full">
                  <div className="marquee-track flex items-center py-2">
                    {[...clientLogos, ...clientLogos].map((img, idx) => (
